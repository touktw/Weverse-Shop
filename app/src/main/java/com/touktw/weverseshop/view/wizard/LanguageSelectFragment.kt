...
class LanguageSelectFragment : BaseWizardFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        textTitle.text = "언어를 선택해 주세요."
        textDescription.text = "사용하실 언어를 선택해 주세요.\n선택한 언어는 More 메뉴에서 언제든 수정할 수 있습니다."
        initRadio()
    }

    private fun updateNextButton() {
        val exists = wizardViewModel?.selectedLocale != null
    ...
    private fun initRadio() {
        wizardViewModel?.locales?.observe(viewLifecycleOwner, Observer { locales ->
            CoroutineScope(Dispatchers.Main).launch {
                ...
                wizardViewModel?.selectedLocale?.let { locale ->
                    wizardViewModel?.locales?.value?.indexOf(locale)?.let {
                        radioGroup.check(it)
                    }
                }
            }
        ...
        radioGroup.setOnCheckedChangeListener { group, checkedId ->
            wizardViewModel?.locales?.value?.get(checkedId)?.let {
                wizardViewModel?.selectedLocale = it
                updateNextButton()
            }
        }
        updateNextButton()
